...
static bool whisper_model_load(struct whisper_model_loader *loader,
  ...
  // load hparams
  {
    ...
    // hparams.n_audio_ctx); fprintf(stderr, "%s: n_audio_state = %d\n",
    // __func__, hparams.n_audio_state); fprintf(stderr, "%s: n_audio_head  =
    // %d\n", __func__, hparams.n_audio_head); fprintf(stderr, "%s:
    // n_audio_layer = %d\n", __func__, hparams.n_audio_layer); fprintf(stderr,
    // "%s: n_text_ctx    = %d\n", __func__, hparams.n_text_ctx);
    // fprintf(stderr, "%s: n_text_state  = %d\n", __func__,
    // hparams.n_text_state); fprintf(stderr, "%s: n_text_head   = %d\n",
    // __func__, hparams.n_text_head); fprintf(stderr, "%s: n_text_layer  =
    // %d\n", __func__, hparams.n_text_layer); fprintf(stderr, "%s: n_mels =
    // %d\n", __func__, hparams.n_mels); fprintf(stderr, "%s: ftype         =
    // %d\n", __func__, model.hparams.ftype); fprintf(stderr, "%s: qntvr =
    ...
    // print memory requirements
    {
      ...
      // fprintf(stderr, "%s: mem required  = %7.2f MB (+ %7.2f MB per
      // decoder)\n", __func__,
      //         mem_required / 1024.0 / 1024.0, mem_required_decoder / 1024.0 /
      //         1024.0);
    }
  ...
  const ggml_type vtype =
      wctx.wtype == GGML_TYPE_F32 ? GGML_TYPE_F32 : GGML_TYPE_F16; // conv type

  {
    ...
    {
      ctx_size +=
          n_audio_ctx * n_audio_state * ggml_type_sizef(GGML_TYPE_F32); // e_pe;

      ctx_size +=
          3 * n_mels * n_audio_state * ggml_type_sizef(vtype);    // e_conv_1_w
    ...
    {
      ctx_size +=
          n_text_ctx * n_text_state * ggml_type_sizef(GGML_TYPE_F32); // d_pe;
    ...
    // decoder layers
    {
      ...
      ctx_size +=
          n_text_layer *
          (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // cross_attn_ln_0_w
      ctx_size +=
          n_text_layer *
          (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // cross_attn_ln_0_b

      ctx_size += n_text_layer * (n_text_state * n_text_state *
                                  ggml_type_sizef(wtype)); // cross_attn_q_w
      ctx_size +=
          n_text_layer *
          (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // cross_attn_q_b
      ...
      ctx_size +=
          n_text_layer *
          (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // cross_attn_v_b

      ctx_size += n_text_layer * (n_text_state * n_text_state *
                                  ggml_type_sizef(wtype)); // cross_attn_ln_1_w
      ctx_size +=
          n_text_layer *
          (n_text_state * ggml_type_sizef(GGML_TYPE_F32)); // cross_attn_ln_1_b
    }
  ...
  // prepare memory for the weights
  {
    ...
    // encoder
    {
      ...
      for (int i = 0; i < n_audio_layer; ++i) {
        ...
        layer.mlp_0_w =
            ggml_new_tensor_2d(ctx, wtype, n_audio_state, 4 * n_audio_state);
        layer.mlp_0_b =
            ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 4 * n_audio_state);

        layer.mlp_1_w =
            ggml_new_tensor_2d(ctx, wtype, 4 * n_audio_state, n_audio_state);
  ...
  // load weights
  {
    ...
    while (true) {
      ...
      // ne[0], ne[1], ne[2], ggml_type_name((ggml_type) ttype),
...
static bool whisper_encode_internal(whisper_context &wctx,
  ...
  if (!use_coreml && !use_openvino) {
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // norm
      {
        ...
        cur = ggml_add(
            ctx0,
            ggml_mul(ctx0, ggml_repeat(ctx0, layer.attn_ln_0_w, cur), cur),
      ...
      // self-attention
      {
        ...
        Qcur = ggml_add(ctx0, ggml_repeat(ctx0, layer.attn_q_b, Qcur), Qcur);

        // Qcur = ggml_scale_inplace(ctx0, Qcur, ggml_new_f32(ctx0,
        // pow(float(n_state)/n_head, -0.25)));

        // note: no bias for Key
        struct ggml_tensor *Kcur = ggml_mul_mat(ctx0, layer.attn_k_w, cur);

        // Kcur = ggml_scale_inplace(ctx0, Kcur, ggml_new_f32(ctx0,
        // pow(float(n_state)/n_head, -0.25)));

        struct ggml_tensor *Vcur = ggml_mul_mat(ctx0, layer.attn_v_w, cur);

        Vcur = ggml_add(ctx0, ggml_repeat(ctx0, layer.attn_v_b, Vcur), Vcur);
            ...
            ggml_cpy(ctx0,
                     ggml_permute(ctx0,
                                  ggml_reshape_3d(ctx0, Vcur, n_state / n_head,
                                                  n_head, n_ctx),
                                  1, 2, 0, 3),
                     ggml_new_tensor_3d(ctx0, wctx.itype, n_ctx,
        ...
        struct ggml_tensor *Q = ggml_permute(
            ctx0,
            ggml_cpy(ctx0, Qcur,
                     ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_state / n_head,
                                        n_head, n_ctx)),
            0, 2, 1, 3);
        ...
        struct ggml_tensor *KQ_scaled = ggml_scale_inplace(
            ctx0, KQ, ggml_new_f32(ctx0, 1.0f / sqrt(float(n_state) / n_head)));

        struct ggml_tensor *KQ_soft_max =
            ggml_soft_max_inplace(ctx0, KQ_scaled);
      ...
      // projection
      {
        ...
        cur = ggml_add(ctx0, ggml_repeat(ctx0, layer.attn_ln_1_b, cur), cur);
      }
      ...
      // feed-forward network
      {
        // norm
        {
          ...
          cur = ggml_add(
              ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, layer.mlp_ln_w, cur), cur),
        ...
        cur = ggml_flash_ff(
            ctx0,
            ggml_cpy(ctx0, cur,
                     ggml_new_tensor_2d(ctx0, wstate.itype, n_state, n_ctx)),
        ...
        cur = ggml_add(ctx0, ggml_repeat(ctx0, layer.mlp_0_b, cur), cur);
        ...
        cur = ggml_add(ctx0, ggml_repeat(ctx0, layer.mlp_1_b, cur), cur);
#endif
      }
  ...
  // pre-compute cross-attention memory
  {
    ...
    for (int il = 0; il < model.hparams.n_text_layer; ++il) {
      ...
      Kcross = ggml_scale_inplace(
          ctx0, Kcross,
          ggml_new_f32(ctx0, pow(float(n_state) / n_head, -0.25)));
      ...
      Vcross = ggml_add(ctx0, ggml_repeat(ctx0, layer.cross_attn_v_b, Vcross),
                        Vcross);
...
static bool whisper_decode_internal(whisper_context &wctx,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // norm
    {
      ...
      // cur = ln_0_w*cur + ln_0_b
      cur = ggml_add(
          ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, layer.attn_ln_0_w, cur), cur),
          ggml_repeat(ctx0, layer.attn_ln_0_b, cur));
    }
    ...
    // self-attention
    {
      ...
      Qcur = ggml_scale_inplace(
          ctx0, Qcur, ggml_new_f32(ctx0, pow(float(n_state) / n_head, -0.25)));

      // note: no bias for Key
      struct ggml_tensor *Kcur = ggml_mul_mat(ctx0, layer.attn_k_w, cur);

      Kcur = ggml_scale_inplace(
          ctx0, Kcur, ggml_new_f32(ctx0, pow(float(n_state) / n_head, -0.25)));
      ...
      // store key and value to memory
      {
        ...
        struct ggml_tensor *k = ggml_view_1d(
            ...
            (ggml_element_size(kv_self.k) * n_state) * (il * n_ctx + n_past));
        struct ggml_tensor *v = ggml_view_2d(
            ctx0, kv_self.v, N, n_state, (n_ctx)*ggml_element_size(kv_self.v),
      ...
      struct ggml_tensor *K = ggml_permute(
          ...
          ggml_reshape_3d(
              ctx0,
              ggml_view_1d(ctx0, kv_self.k, (n_past + N) * n_state,
                           il * n_ctx * ggml_element_size(kv_self.k) * n_state),
      ...
      struct ggml_tensor *KQ_masked =
          ggml_diag_mask_inf_inplace(ctx0, KQ, n_past);

      struct ggml_tensor *KQ_soft_max = ggml_soft_max_inplace(ctx0, KQ_masked);

      struct ggml_tensor *V =
          ggml_view_3d(ctx0, kv_self.v, n_past + N, n_state / n_head, n_head,
    ...
    // norm
    {
      ...
      cur = ggml_add(
          ctx0,
          ggml_mul(ctx0, ggml_repeat(ctx0, layer.cross_attn_ln_0_w, cur), cur),
    ...
    // cross-attention
    {
      ...
      Qcur =
          ggml_add(ctx0, ggml_repeat(ctx0, layer.cross_attn_q_b, Qcur), Qcur);
      ...
      struct ggml_tensor *Kcross = ggml_reshape_3d(
          ctx0,
          ggml_view_1d(ctx0, wstate.kv_cross.k, M * n_state,
                       il * M * ggml_element_size(wstate.kv_cross.k) * n_state),
      ...
      //             ggml_new_tensor_3d(ctx0, Vcross->type, M, n_state/n_head,
      //             n_head));
    ...
    // projection
    {
      ...
      cur =
          ggml_add(ctx0, ggml_repeat(ctx0, layer.cross_attn_ln_1_b, cur), cur);
    }
...
struct whisper_state *whisper_init_state(whisper_context *ctx) {
...

  {
    ...
    // ggml_nbytes(state->decoders[0].kv_self.v); silence fprintf(stderr, "%s:
    // kv self size  = %7.2f MB\n", __func__, memory_size / 1024.0 / 1024.0);
  }
  ...
  // {
  // const size_t memory_size = ggml_nbytes(state->kv_cross.k) +
  // ggml_nbytes(state->kv_cross.v); fprintf(stderr, "%s: kv cross size = %7.2f
  // MB\n", __func__, memory_size / 1024.0 / 1024.0);
  // }
...
int whisper_ctx_init_openvino_encoder(struct whisper_context *ctx,
  ...
  if (!model_path && ctx->path_model.empty()) {
    fprintf(stderr,
            "%s: model_path is nullptr, and ctx has no model_path set.\n",
            __func__);
    return 1;
  }
    ...
    for (int it = 0; it < (int)temperatures.size(); ++it) {
      ...
      for (int i = 0, n_max = whisper_n_text_ctx(ctx) / 2 - 4; i < n_max; ++i) {
        ...
        for (int j = 0; j < n_decoders_cur; ++j) {
          ...
          case whisper_sampling_strategy::WHISPER_SAMPLING_BEAM_SEARCH: {
            ...
            for (const auto &token : tokens_new) {
              ...
              // __func__, ctx->vocab.id_to_token.at(token.id).c_str(),
              // token.plog, beam_candidates.back().sequence.sum_logprobs_all);
            }
          } break;
          };
        }
        ...
        for (int j = 0; j < n_decoders_cur; ++j) {
          ...
          // WHISPER_PRINT_DEBUG("%s: decoder %d: token %d, kv_self.n %d,
      ...
      if (it != (int)temperatures.size() - 1 &&
        ...
        if (success) {
          ...
          // {
          //     WHISPER_PRINT_DEBUG("%s: token = %d, p = %6.3f, pt = %6.3f, ts
...
WHISPER_API const char *whisper_bench_ggml_mul_mat_str(int n_threads) {
  ...
  for (int j = 0; j < (int)sizes.size(); j++) {
    ...
    snprintf(strbuf, sizeof(strbuf),
             "%4zu x %4zu: Q4_0 %7.1f GFLOPS (%3d runs) | Q4_1 %7.1f GFLOPS "
             "(%3d runs)\n",
    ...
    // Q5_0 | Q5_1 | Q8_0
    snprintf(strbuf, sizeof(strbuf),
             "%4zu x %4zu: Q5_0 %7.1f GFLOPS (%3d runs) | Q5_1 %7.1f GFLOPS "
             "(%3d runs) | Q8_0 %7.1f GFLOPS (%3d runs)\n",
             N, N, s_q5_0, n_q5_0, s_q5_1, n_q5_1, s_q8_0, n_q8_0);
    s += strbuf;

    // F16 | F32
    snprintf(strbuf, sizeof(strbuf),
             "%4zu x %4zu: F16  %7.1f GFLOPS (%3d runs) | F32  %7.1f GFLOPS "
             "(%3d runs)\n",
